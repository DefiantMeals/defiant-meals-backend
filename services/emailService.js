...
// Send Grab & Go order confirmation to customer
const sendGrabAndGoConfirmation = async (order) => {
  try {
    ...
    if (!customerEmail) {
      console.error('No customer email found in Grab & Go order');
      return false;
    }
    ...
    const emailData = {
      from: 'Defiant Meals <orders@defiantmeals.com>',
      to: [customerEmail],
      subject: `Grab & Go Order Confirmation #${order._id.toString().slice(-8)}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          <h1 style="color: #059669; text-align: center;">Grab & Go Order Confirmed!</h1>
    ...
    console.log('Attempting to send Grab & Go customer email to:', customerEmail);
    const { data, error } = await resend.emails.send(emailData);

    if (error) {
      console.error('Error sending Grab & Go customer email:', error);
      return false;
    }

    console.log('Grab & Go confirmation sent to customer:', customerEmail, 'Email ID:', data.id);
    return true;
  } catch (error) {
    console.error('Error in sendGrabAndGoConfirmation:', error);
    return false;
  }
};

// Send Grab & Go order notification to admin
const sendGrabAndGoAdminNotification = async (order) => {
  try {
    const customerEmail = order.customerEmail;
    ...
    const emailData = {
      from: 'Defiant Meals <orders@defiantmeals.com>',
      to: ['defiantmealsmenu@gmail.com'],
      subject: `🛒 New Grab & Go Order #${order._id.toString().slice(-8)} - $${totalAmount.toFixed(2)}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          <h1 style="color: #059669; text-align: center;">🛒 New Grab & Go Order!</h1>
          ...
          <div style="text-align: center; margin: 30px 0; padding: 20px; background: #eff6ff; border-radius: 8px;">
            <p style="font-size: 16px;">Check your admin dashboard to manage this order.</p>
            <a href="https://defiantmeals.com/admin" style="display: inline-block; background: #059669; color: white; padding: 10px 20px; text-decoration: none; border-radius: 4px; margin-top: 10px;">View Dashboard</a>
          </div>
        </div>
      `
    };
    ...
    if (error) {
      console.error('Error sending Grab & Go admin notification:', error);
      return false;
    }

    console.log('Grab & Go admin notification sent:', data.id);
    return true;
  } catch (error) {
    console.error('Error in sendGrabAndGoAdminNotification:', error);
    return false;
  }
};
...
module.exports = {
  ...
  sendGrabAndGoConfirmation,
  sendGrabAndGoAdminNotification
};